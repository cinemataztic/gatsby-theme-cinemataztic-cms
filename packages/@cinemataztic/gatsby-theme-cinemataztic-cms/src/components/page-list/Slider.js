/**
 * Created by @author @ddennis - ddennis.dk aka fantastisk.dk/works aka meresukker.dk on 23-08-2019.
 */
...
export default function Slider({ items, width = 600, visible = 4, style, children }) {
	   ...
	   (y, vy) => {
		   ...
		   set(i => {
			   ...
			   return {
				   ...
				   config: { tension: (1 + items.length - configPos) * 150, friction: 30 + configPos * 40 }
	...
	const bind = useGesture({
		onDrag: ({ local: [x], vxvy: [vx] }) => vx && ((dragOffset.current = -x), runSprings(wheelOffset.current + -x, -vx)),
		onWheel: ({ local: [, y], vxvy: [, vy] }) => vy && ((wheelOffset.current = y), runSprings(dragOffset.current + y, vy))
	})

	return (
	   <div {...bind()} style={{ ...style, ...styles.container , userSelect:"none"}}>
		   {springs.map(({ x, vel }, i) => (
			  <a.div key={i} style={{ ...styles.item, width, transform: transform(x) }} children={children(items[i], i)} />
		   ))}
	   </div>
	)
}
