...
const FullWidthComp = (props) => {
  ...
  const showDivider = (!text && !title) || hasTextAlign === 'text-center' ? false : true
  ...
  return (
    <Waypoint onEnter={onEnter} onLeave={onLeave}>
      <div ref={myElement} className="row page-component full-width-comp mb-5 pb-5 mt-5" style={{ opacity: 0 }}>
        <div className="col-12 col-md-10 mx-auto">
          <div className="row" style={{}}>
            {image && (
              <div className="col-12">
                <GatsbyImage image={getImage(image)} alt="full width component" />
              </div>
            )}
              ...
              <div className="pl-0 pt-5">
                {title && (
                  <div className={adjustCenterText}>
                    <h1 className={`font-weight-bold text-uppercase ${hasTextAlign} `}>{title}</h1>
                  </div>
                )}
                ...
                {text && (
                  <div className={adjustCenterText}>
                    <p className={`body-text ${hasTextAlign}`} style={{ whiteSpace: 'pre-wrap' }}>
                      {text}
                    </p>
                  </div>
                )}
